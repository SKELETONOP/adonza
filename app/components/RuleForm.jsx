...
function ItemCard({ item, onChange, onRemove }) {
  ...
  const pick = async () => {
    ...
    const productImage = chosen.images?.[0]?.originalSrc;
    const variants = (chosen.variants || []).map((v) => ({
      id: v.id,
...
function ItemList({ items, setItems, addLabel }) {
  return (
    <BlockStack gap="300">
...
export default function RuleForm({
  ...
  const [triggerBasis, setTriggerBasis] = useState(
            ...
            <Card>
              <BlockStack gap="400">
                ...
                <Select
                  ...
                  value={triggerBasis}
                  onChange={(value) => setTriggerBasis(value)}
                />
